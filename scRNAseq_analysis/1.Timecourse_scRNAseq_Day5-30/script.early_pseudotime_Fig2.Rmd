...
p1 <- UMAPPlot(Timecourse, group.by="class3", cols=color_anno) & NoAxes()
p2 <- UMAPPlot(Timecourse, group.by="dataset",  cols=met.brewer("Moreau",6)) & NoAxes()
...
p1 <- UMAPPlot(Timecourse, group.by="dataset", cols = c("#3F1B0C","#88271B","#BB762A")) & NoAxes()
p1

feature_p1 <- FeaturePlot(Timecourse, c("IGFBP5","HAPLN3","COL4A1","COL1A1","FAT1"), ncol=5, pt.size = 0.5,order=T,cols= c("grey90",colorRampPalette(c("grey95","grey80" ,"black"))(50)))  & NoAxes() &NoLegend()
feature_p1
...
p3 <- UMAPPlot(Timecourse, group.by="class3", cols = c("grey90","grey10")) & NoAxes()
...
p5 <- ggplot(Timecourse@meta.data, aes(x = dc1_ranked, colour = class3, fill = class3)) +
...
Cyan_red_colscheme <- colorRampPalette(rev(c("#3F1B0C","#88271B","#BB762A")))
...
res.pt_dep_relaxed <- subset(res_pt_dep, is_pt_dep_relaxed == "TRUE")
...
meta_genes <- getBM(attributes = c("ensembl_gene_id",
                                   ...
                                   "strand"),
                    filters = "hgnc_symbol",
                    values = res_pt_dep$gene,
                    mart = ensembl) %>%
  right_join(data.frame(hgnc_symbol= res_pt_dep$gene),
             by = "hgnc_symbol") %>%
