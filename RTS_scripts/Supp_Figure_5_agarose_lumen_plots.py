...
def to_long_form(input_data, timepoints, condition, modality):
    ...
    df_converted['image_number'] = df_converted['variable'].str.extract(
        r'(\d+)')
...
organoid_samples = ['organoid (matrigel) 1', 'organoid (matrigel) 2',
                    'organoid (matrigel) 3', 'organoid (matrigel) 4',
                    'organoid (no ecm) 5', 'organoid (no ecm) 6',
                    'organoid (no ecm) 7', 'organoid (no ecm) 8',
                    'organoid (in agar) 9', 'organoid (in agar) 10',
                    'organoid (in agar) 11', 'organoid (in agar) 12',
                    'organoid (no ecm) 13', 'organoid (no ecm) 14',
                    'organoid (no ecm) 15', 'organoid (no ecm) 16']

df_agar_lumen_volume = to_long_form(lumen_volume_df_agar_06,
                                    timepoints='Unnamed: 0',
                                    condition=organoid_samples,
                                    modality='lumen_volume')

df_agar_number_of_lumen = to_long_form(n_lumen_df_agar_06,
                                       timepoints='Unnamed: 0',
                                       condition=organoid_samples,
                                       modality='number_of_lumen')

df_agar_organoid_volume = to_long_form(organoid_volume_df_agar_06,
                                       timepoints='Unnamed: 0',
                                       condition=organoid_samples,
...
df_all_modalities = df_all_modalities.join(
    df_agar_organoid_volume['organoid_volume'])
