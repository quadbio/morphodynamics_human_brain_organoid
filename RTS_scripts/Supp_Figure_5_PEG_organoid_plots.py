...
df_all_conditions = pd.concat([df_all_conditions, res_df_RDG_plus_laminin],
                              ignore_index=True)
df_all_conditions = pd.concat([df_all_conditions, res_df_GFO_minus],
                              ignore_index=True)
...
df_all_conditions['ratio_lumen_o_organoid'] = (
    df_all_conditions['lumen_volume'] /
    df_all_conditions['organoid_volume']
    )

# (lumen volume)/(lumen+epithelium volume)
df_all_conditions['percent_lumen_vol'] = (
    df_all_conditions['lumen_volume'] /
    (df_all_conditions['lumen_volume'] +
     df_all_conditions['organoid_volume'])
     )
...
sns.lineplot(df_all_conditions, x='time_in_days', y='number_of_lumen',
             hue='Sample', errorbar='sd', palette=custom_palette.values())
...
sns.lineplot(df_all_conditions, x='time_in_days', y='lumen_volume',
             hue='Sample', errorbar='sd', palette=custom_palette.values())
plt.xlabel('time[days]')
