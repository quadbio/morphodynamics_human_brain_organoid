...
def run_cellpose(sample:str,
    ...
    if config['cellpose']['segment_dual_channel']:
        dir_images_sample = Path(dir_images, sample)
        df_images = get_metadata(dir_images_sample, custom_regex=config['regex'])
        file_nuclei = df_images[(df_images['channel_id'] == config['channel_nuclei']) &
                         (df_images['cycle_id'] == config['reference_cycle'])]['file'].values[0]
    ...
    if pretrained_model!=False:
        if config['cellpose']['segment_dual_channel']:
            ...
            img_nuclei = io.imread(Path(dir_images_sample, file_nuclei))
            img=np.array([img,img_nuclei])
            mask, flows, styles = model.eval(img, diameter=diameter, channels=[[1, 2]], do_3D=False,
