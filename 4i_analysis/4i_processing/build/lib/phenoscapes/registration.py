...
def run_elastix(sample: str,
    ...
    for cycle in cycles:
        ...
        if config['alignment']['mask_bright_spots']:
            bright_spot_mask = moving_img.copy()
            #Create bright spots mask --> 97 percentile
            bright_spot_mask = ~ndimage.binary_fill_holes(bright_spot_mask>np.percentile(bright_spot_mask,97))
            #Zero out bright spots
            moving_img=moving_img*bright_spot_mask
            print('Masked bright spots.')
        ...
        for channel in channels:
            ...
            if (channel == ch_nuclei) and (config['alignment']['mask_bright_spots']==False):
