...
def extract_contours(mask,scale=10):
    contour_normals = []
    contour_starts = []
    for sub_mask in np.unique(mask)[1:]:
        one_mask=(mask==sub_mask).astype(np.uint8)
        contours, _ = cv2.findContours(one_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        contours=contours[0]
        contour_normal=[]
        contour_start=[]
...
def extract_angles(mask,cell_masks,df,scale=10):
    contour_normals,contour_starts=extract_contours(mask,scale)
    cosine_sims=[]  
...
def run_extract_morphology_features(sample:str,
    ...
    if dir_segmented_cells!=None:
        masks_whole_cell= io.imread(Path(dir_segmented_cells, sample + '.tif'))
        if config["speckle_removal"]["run"]:
            ...
            tissue_mask=(masks_whole_cell>0).astype(float)
    ...
    #Ensure that we only take the largest mask
    labelled_mask = label(tissue_mask)
    measurement_labelled = regionprops(labelled_mask)
    size_max = [prop.area for prop in measurement_labelled]
    label_names = [prop.label for prop in measurement_labelled]
    tissue_mask=(labelled_mask==label_names[np.argmax(size_max)])
    
    #Extract distance + angles
